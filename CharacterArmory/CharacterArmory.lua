...
local ElvUI_BagModule, ElvUI_DataBars = E:GetModule("Bags"), E:GetModule("DataBars")
...
function CA:Setup_CharacterArmory()
	...
	for i, SlotName in pairs(Info.Armory_Constants.GearList) do
		...
		Slot.Gradation:SetTexture('Interface\\AddOns\\ElvUI_AdvancedArmory\\Media\\Textures\\Gradation')
		...
		if not E.db.kfAA.Armory.Character.Gradation.Display then
			Slot.Gradation:Hide()
		end
		...
		if SlotName ~= 'ShirtSlot' and SlotName ~= 'TabardSlot' then
			...
			KF:TextSetting(Slot, nil, { Tag = 'ItemLevel',
				Font = E.db.kfAA.Armory.Character.Level.Font,
				FontSize = E.db.kfAA.Armory.Character.Level.FontSize,
				FontStyle = E.db.kfAA.Armory.Character.Level.FontStyle,
			...
			if E.db.kfAA.Armory.Character.Level.Display == 'Hide' then
				Slot.ItemLevel:Hide()
			end
			
			-- Enchantment Name
			KF:TextSetting(Slot, nil, { Tag = 'ItemEnchant',
				Font = E.db.kfAA.Armory.Character.Enchant.Font,
				FontSize = E.db.kfAA.Armory.Character.Enchant.FontSize,
				FontStyle = E.db.kfAA.Armory.Character.Enchant.FontStyle,
				directionH = Slot.Direction
			}, Slot.Direction, _G["Character"..SlotName], Slot.Direction == 'LEFT' and 'RIGHT' or 'LEFT', Slot.Direction == 'LEFT' and 2 or -2, 1)
			
			if E.db.kfAA.Armory.Character.Enchant.Display == 'Hide' then
				Slot.ItemEnchant:Hide()
			end
			
			Slot.EnchantWarning = CreateFrame('Button', nil, Slot)
			Slot.EnchantWarning:Size(E.db.kfAA.Armory.Character.Enchant.WarningSize)
			Slot.EnchantWarning.Texture = Slot.EnchantWarning:CreateTexture(nil, 'OVERLAY')
			Slot.EnchantWarning.Texture:SetInside()
			Slot.EnchantWarning.Texture:SetTexture('Interface\\AddOns\\ElvUI_AdvancedArmory\\Media\\Textures\\Warning-Small')
			...
			KF:TextSetting(Slot, nil, { Tag = 'Durability',
				Font = E.db.kfAA.Armory.Character.Durability.Font,
				FontSize = E.db.kfAA.Armory.Character.Durability.FontSize,
				FontStyle = E.db.kfAA.Armory.Character.Durability.FontStyle,
				directionH = Slot.Direction
			...
			for i = 1, MAX_NUM_SOCKETS do
				...
				Slot["Socket"..i]:Size(E.db.kfAA.Armory.Character.Gem.SocketSize)
			...
			Slot.SocketWarning:Size(E.db.kfAA.Armory.Character.Enchant.WarningSize)
			...
			Slot.SocketWarning.Texture:SetTexture('Interface\\AddOns\\ElvUI_AdvancedArmory\\Media\\Textures\\Warning-Small')
			...
			if Info.Armory_Constants.CanTransmogrifySlot[SlotName] then
				...
				Slot.TransmogrifyAnchor.Texture:SetTexture('Interface\\AddOns\\ElvUI_AdvancedArmory\\Media\\Textures\\Anchor')
	...
	if E.private.kfAA.Armory.UseArtMonitor then -- Legion : Artifact Weapon Monitor
		...
		self.ArtifactMonitor.SocketWarning:Size(E.db.kfAA.Armory.Character.Enchant.WarningSize)
		...
		KF:TextSetting(self.ArtifactMonitor, ARTIFACT_POWER..' : ', { Tag = 'PowerTitle',
			Font = E.db.kfAA.Armory.Character.Artifact.Font,
			FontSize = E.db.kfAA.Armory.Character.Artifact.FontSize,
			FontStyle = E.db.kfAA.Armory.Character.Artifact.FontStyle,
			directionH = 'LEFT'
...
function CA:Update_Durability()
	...
	for _, SlotName in pairs(Info.Armory_Constants.GearList) do
		...
		if CurrentDurability and MaxDurability and not (E.db.kfAA.Armory.Character.Durability.Display == 'DamagedOnly' and CurrentDurability == MaxDurability) then
			...
			if (E.db.kfAA.Armory.Character.Durability.Display == 'MouseoverOnly' and not Slot:IsMouseOver()) or E.db.kfAA.Armory.Character.Durability.Display == 'Hide' then
...
function CA:ConstructArtSockets() --Creating gem slots for artifact. Apparently having it to be done on load actually fails
	...
	for i = 1, C_ArtifactUI.GetEquippedArtifactNumRelicSlots() or 3 do
		...
		CA.ArtifactMonitor['Socket'..i]:Size(E.db.kfAA.Armory.Character.Gem.SocketSize)
...
function CA:Update_Gear()
	...
	for _, SlotName in pairs(type(self.GearUpdated) == 'table' and self.GearUpdated or Info.Armory_Constants.GearList) do
		...
		if not (SlotName == 'ShirtSlot' or SlotName == 'TabardSlot') then
			...
			if ItemLink then
				if not ItemLink:find('%[%]') then -- sometimes itemLink is malformed so we need to update when crashed
					...
					ItemData = { string.split(':', ItemLink) }
					...
					do --<< Gem Parts >>--
						...
						if self.ArtifactMonitor and Legion_ArtifactData.ItemID and Legion_ArtifactData.MajorSlot == SlotName then
							...
							for i = 1, Slot.GemCount_Enable do
								...
								if GemLink then GemTexture = select(10, GetItemInfo(GemLink)) end
								...
								if E.db.kfAA.Armory.Character.Gem.Display == 'Always' or E.db.kfAA.Armory.Character.Gem.Display == 'MouseoverOnly' and Slot.Mouseovered or E.db.kfAA.Armory.Character.Gem.Display == 'MissingOnly' then
									Slot['Socket'..i]:Show()
								...
								--<< Notice Missing >>--
								if E.db.kfAA.Armory.Character.NoticeMissing ~= false and Slot.GemCount_Enable > GemCount_Now or Slot.GemCount_Enable > GemCount or GemCount_Now > GemCount then
							...
							for i = 1, MAX_NUM_SOCKETS do
								...
								if GemTexture and GemTexture:find('Interface\\ItemSocketingFrame\\') then
									...
									Slot["Socket"..GemCount_Default].GemType = string.upper(gsub(GemTexture, 'Interface\\ItemSocketingFrame\\UI--EmptySocket--', ''))
								end
							end
						...
						elseif CurrentLineText:find(Info.Armory_Constants.EnchantKey) then
							if E.db.kfAA.Armory.Character.Enchant.Display ~= 'Hide' then
								...
								for Name, _ in pairs(AA_ArmoryDB.EnchantString) do
									if AA_ArmoryDB.EnchantString[Name].original and AA_ArmoryDB.EnchantString[Name].new then
										CurrentLineText = gsub(CurrentLineText, AA_ArmoryDB.EnchantString[Name].original, AA_ArmoryDB.EnchantString[Name].new)
									end
								end
					...
					if BasicItemLevel then
						if ItemUpgradeID then
							if ItemUpgradeID == '' or not E.db.kfAA.Armory.Character.Level.ShowUpgradeLevel and ItemRarity == 7 then
								ItemUpgradeID = nil
						...
						Slot.ItemLevel:SetText(
							...
							or
							E.db.kfAA.Armory.Character.Level.ShowUpgradeLevel and (Slot.Direction == 'LEFT' and TrueItemLevel..' ' or '')..(ItemUpgradeID and (Info.Armory_Constants.UpgradeColor[ItemUpgradeID] or '|cffaaaaaa')..'(+'..ItemUpgradeID..')|r' or '')..(Slot.Direction == 'RIGHT' and ' '..TrueItemLevel or '')
							or
							TrueItemLevel
						)
					end
					
					if E.db.kfAA.Armory.Character.NoticeMissing ~= false then
						if not Slot.IsEnchanted and 
							...
							if not E.db.kfAA.Armory.Character.Enchant.WarningIconOnly then
						...
						if E.db.kfAA.Armory.Character.MissingIcon then
					...
					if Slot.TransmogrifyAnchor and C_Transmog_GetSlotInfo(Slot.ID, LE_TRANSMOG_TYPE_APPEARANCE) then
						Slot.TransmogrifyAnchor.Link = select(6, C_TransmogCollection_GetAppearanceSourceInfo(select(3, C_Transmog_GetSlotVisualInfo(Slot.ID, LE_TRANSMOG_TYPE_APPEARANCE))));
					...
					if Slot.IllusionAnchor then
						...
						if IsTransmogrified then
							...
							_, _, Slot.IllusionAnchor.Link = C_TransmogCollection_GetIllusionSourceInfo(select(3, C_Transmog_GetSlotVisualInfo(Slot.ID, LE_TRANSMOG_TYPE_ILLUSION)))
		...
		-- Change Gradation
		if ItemLink and E.db.kfAA.Armory.Character.Gradation.Display then
			Slot.Gradation:Show()
		else
			Slot.Gradation:Hide()
		end
		if ErrorDetected and E.db.kfAA.Armory.Character.NoticeMissing then
			Slot.Gradation:SetVertexColor(1, 0, 0)
			Slot.Gradation:Show()
		else
			Slot.Gradation:SetVertexColor(unpack(E.db.kfAA.Armory.Character.Gradation.Color))
		end
	end
...
do --<< Artifact Monitor >>
		...
		if Legion_ArtifactData.ItemID then
			...
			if Legion_ArtifactData.MajorSlot == 'SecondaryHandSlot' or self.SecondaryHandSlot.ItemRarity == 6 then
				...
				do	-- Notice Missing
					...
					if E.db.kfAA.Armory.Character.NoticeMissing then
	...
	function CA:LegionArtifactMonitor_UpdateData()
		...
		if Artifact_ItemID then
			...
			if E.db.kfAA.Armory.Character.Artifact.ShortValues then
				Legion_ArtifactData.Power = E:ShortValue(Legion_ArtifactData.XP)
			else
				Legion_ArtifactData.Power = Legion_ArtifactData.XP
			end
			
			if E.db.kfAA.Armory.Character.Artifact.ShortValues then Legion_ArtifactData.XPForNextPointShort = E:ShortValue(Legion_ArtifactData.XPForNextPoint) end
			...
			if Legion_ArtifactData.AvailablePoint > 0 then
				self.ArtifactMonitor.BarExpected.Message:SetText(format("%s Point(s) Available!!", KF:Color_Value(Legion_ArtifactData.AvailablePoint)))
			else
...
function CA:Update_BG()
	if E.db.kfAA.Armory.Character.Backdrop.SelectedBG == 'HIDE' then
		self.BG:SetTexture(nil)
	elseif E.db.kfAA.Armory.Character.Backdrop.SelectedBG == 'CUSTOM' then
		self.BG:SetTexture(E.db.kfAA.Armory.Character.Backdrop.CustomAddress)
	else
		self.BG:SetTexture(Info.Armory_Constants.BlizzardBackdropList[E.db.kfAA.Armory.Character.Backdrop.SelectedBG] or 'Interface\\AddOns\\ElvUI_AdvancedArmory\\Media\\Textures\\'..E.db.kfAA.Armory.Character.Backdrop.SelectedBG)
	end
end
...
function CA:Update_Display(Force)
	...
	if (_G["PaperDollFrame"]:IsMouseOver() and (E.db.kfAA.Armory.Character.Level.Display == 'MouseoverOnly' or E.db.kfAA.Armory.Character.Enchant.Display == 'MouseoverOnly' or E.db.kfAA.Armory.Character.Durability.Display == 'MouseoverOnly' or E.db.kfAA.Armory.Character.Gem.Display == 'MouseoverOnly')) or Force then
		for _, SlotName in pairs(Info.Armory_Constants.GearList) do
			Slot = self[SlotName]
			Mouseover = Slot:IsMouseOver()
			
			if Slot.ItemLevel then
				if E.db.kfAA.Armory.Character.Level.Display == 'Always' or Mouseover and E.db.kfAA.Armory.Character.Level.Display == 'MouseoverOnly' then
					Slot.ItemLevel:Show()
				else
					Slot.ItemLevel:Hide()
				end
			...
			if Slot.ItemEnchant then
				if E.db.kfAA.Armory.Character.Enchant.Display == 'Always' or Mouseover and E.db.kfAA.Armory.Character.Enchant.Display == 'MouseoverOnly' then
					Slot.ItemEnchant:Show()
				elseif E.db.kfAA.Armory.Character.Enchant.Display ~= 'Always' and not (E.db.kfAA.Armory.Character.NoticeMissing and not Slot.IsEnchanted) then
					Slot.ItemEnchant:Hide()
				end
			end
			
			if Slot.Durability then
				if E.db.kfAA.Armory.Character.Durability.Display == 'Always' or Mouseover and E.db.kfAA.Armory.Character.Durability.Display == 'MouseoverOnly' or E.db.kfAA.Armory.Character.Durability.Display == 'DamagedOnly' then
					Slot.Durability:Show()
					
					if Slot.Socket1 then
						if Slot.Durability:GetText() == '' or E.db.kfAA.Armory.Character.Durability.Display == 'MouseoverOnly' and not Mouseover then
							Slot.Socket1:Point('BOTTOM'..Slot.Direction, _G["Character"..SlotName], 'BOTTOM'..(Slot.Direction == 'LEFT' and 'RIGHT' or 'LEFT'), Slot.Direction == 'LEFT' and 2 or -2, 2)
						else
							Slot.Socket1:Point('BOTTOM'..Slot.Direction, Slot.Durability, 'BOTTOM'..(Slot.Direction == 'LEFT' and 'RIGHT' or 'LEFT'), Slot.Durability:GetText() and (Slot.Direction == 'LEFT' and 3 or -1) or 0, Slot.Durability:GetText() and -1 or 0)
						end
			...
			if Slot.Socket1 then
				for i = 1, MAX_NUM_SOCKETS do
					if E.db.kfAA.Armory.Character.Gem.Display == 'Always' or Mouseover and E.db.kfAA.Armory.Character.Gem.Display == 'MouseoverOnly' then
						...
						if Slot["Socket"..i].GemType and E.db.kfAA.Armory.Character.NoticeMissing and not Slot["Socket"..i].GemItemID then
							SocketVisible = true
						end
					end
				end
...
function CA:UpdateSettings(part)
	local db = E.db.kfAA.Armory.Character
...
function CA:UpdateIlvlFont()
	local db = E.db.kfAA.Armory.Character.Stats.ItemLevel
...
function CA:ElvOverlayToggle()
	if E.db.kfAA.Armory.Character.Backdrop.Overlay then
...
KF.Modules.CharacterArmory = function()
	if E.private.kfAA.Armory then E.db.kfAA.Armory.Character.ItemLevel = E.private.kfAA.Armory.ItemLevel; E.db.kfAA.Armory.ItemLevel = nil end --DB converts
	if E.db.kfAA.Armory.Character.Enable ~= false then
	...
	CA:ElvOverlayToggle()
	if KF._Compatibility["DejaCharacterStats"] then
