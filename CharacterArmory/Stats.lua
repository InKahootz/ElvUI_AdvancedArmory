...
-- PaperDollFrame_SetLabelAndText Format Change
	...
	statFrame.tooltip = HIGHLIGHT_FONT_COLOR_CODE..format(PAPERDOLLFRAME_TOOLTIP_FORMAT, BLOCK_CHANCE).." "..format("%.2f", chance).."%"..FONT_COLOR_CODE_CLOSE;
...
function CA:ToggleStats()
	CA:ResetAllStats()
	for _, value in pairs(PAPERDOLL_AttributesIndexDefaultStats) do
		local checked = E.db.kfAA.Armory.Character.Stats.List[value]
		if checked then
			tinsert(PAPERDOLL_STATCATEGORIES[1].stats, { stat = format("%s", value) })
		end
	end
...
function CA:PaperDollFrame_UpdateStats()
	...
	if E.db.kfAA.Armory.Character.Stats.IlvlFull then
		if E.db.kfAA.Armory.Character.Stats.IlvlColor then
			local R, G, B = E:ColorGradient((equipped / total), 1, 0, 0, 1, 1, 0, 0, 1, 0)
			local avColor = E.db.kfAA.Armory.Character.Stats.AverageColor
	...
	for catIndex = 1, #PAPERDOLL_STATCATEGORIES do
		...
		for statIndex = 1, #PAPERDOLL_STATCATEGORIES[catIndex].stats do
			...
			if ( showStat and stat.primary ) then
				...
				if ( stat.primary ~= primaryStat ) and E.db.kfAA.Armory.Character.Stats.OnlyPrimary then
					showStat = false;
				end
			end
