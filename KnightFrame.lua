...
if not (select(2, GetAddOnInfo('ElvUI_KnightFrame')) and IsAddOnLoaded('ElvUI_KnightFrame')) then
	...
	local KF = E:NewModule(AddOnName, 'AceEvent-3.0', 'AceConsole-3.0', 'AceHook-3.0')
	KF.Modules = {}
	...
	for i = 1, #_CompList do
		if GetAddOnEnableState(E.myname, _CompList[i]) == 0 then KF._Compatibility[_CompList[i]] = nil else KF._Compatibility[_CompList[i]] = true end
	end
	...
	if type(AA_ArmoryDB) ~= 'table' then
		AA_ArmoryDB = {
			EnchantString = {}
		}
	end
	...
	function KF:Initialize()
		...
		function KF:ForUpdateAll()
			...
			if not KF._Compatibility["DejaCharacterStats"] then
